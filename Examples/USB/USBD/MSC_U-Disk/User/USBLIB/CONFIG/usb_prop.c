...
uint8_t Request = 0;
volatile uint8_t UDisk_Max_Lun = 0;
...
RESULT USBD_Data_Setup(uint8_t RequestNo)
{
  ...
  else if (Type_Recipient == (CLASS_REQUEST | INTERFACE_RECIPIENT) )
  {
    if (Request_No == 0xFE)
    {
      ...
      pInformation->Ctrl_Info.CopyData = &USBD_UDisk_Get_Max_Lun;
        return USB_SUCCESS;
    }
    else
    {
...
uint8_t *USBD_UDisk_Get_Max_Lun(uint16_t Length)
{
