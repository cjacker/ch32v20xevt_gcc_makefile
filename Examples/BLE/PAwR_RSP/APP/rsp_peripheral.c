...
// GAP GATT Attributes
static uint8_t attDeviceName[] = "esl-test1";
...
uint16_t Peripheral_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & SBP_SET_SYNC_PARAM)
    {
        gapSyncTransferParam_t param;

        param.connHandle = peripheralConnList.connHandle;
        param.mode = MODE_2;
        param.syncTimeout = 1000;
        PRINT("set sync transfer parameters-%x\n", GAPRole_SyncTransferParameters(&param));
        return (events ^ SBP_SET_SYNC_PARAM);
    }
...
static void Peripheral_ProcessGAPMsg(gapRoleEvent_t *pEvent)
{
    switch(pEvent->gap.opcode)
    {
        ...
        case GAP_PERI_ADV_SYNC_TRAN_RECEIVED_EVENT:
        {
            PRINT("sync transfer :%x %d %d.\n", pEvent->syncTran.syncHandle, pEvent->syncTran.numSubevents, pEvent->syncTran.subInterval);
            syncHandle = pEvent->syncTran.syncHandle;
...
static void Peripheral_LinkEstablished(gapRoleEvent_t *pEvent)
{
    ...
    PRINT("---------------sync handle=%x-------------------\n", event->syncHandle);
    if(event->syncHandle != 0xFFFF)
    {
        GAPRole_TerminateSync(event->syncHandle);
    }
}
...
static void simpleProfileChangeCB(uint8_t paramID, uint8_t *pValue, uint16_t len)
{
    switch(paramID)
    {
        case SIMPLEPROFILE_CHAR1:
        {
            ...
            {
                syncTran = 1;
                tmos_set_event(Peripheral_TaskID, SBP_SET_SYNC_PARAM);
            }
            break;
        }
