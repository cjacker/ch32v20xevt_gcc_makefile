...
// Peer device address
uint8_t PeerAddrDef[B_ADDR_LEN] = {0x02, 0x02, 0x03, 0xE4, 0xC2, 0x84};
...
static uint8_t discoveryFlag = 0;
static uint8_t syncTran = 0;
...
uint16_t Central_ProcessEvent(uint8_t task_id, uint16_t events)
{
                ...
                if(req.pValue != NULL)
                {
                    *req.pValue = centralCharVal;
                    ...
                    if(GATT_WriteCharValue(centralConnHandle, &req, centralTaskId) == SUCCESS)
                    {
                        ...
                        {
                            syncTran = 1;
                            tmos_start_task(centralTaskId, START_PERIODIC_T_TEST, 1600);
                        }
                    }
                    else
                    {
    ...
    if(events & START_PERIODIC_T_TEST)
    {
        gapSyncTransferAdvertising_t param;
        bStatus_t                    s;

        param.connHandle = centralConnHandle;
        param.serviceData = 0x55;
        param.advHandle = 1;
        s = GAPRole_SyncTransferAdvertising(&param);
        PRINT("----   sync transfer sync-%x.  -----\n", s);
        return (events ^ START_PERIODIC_T_TEST);
    }
    return 0;
}
...
static void centralEventCB(gapRoleEvent_t *pEvent)
{
    switch(pEvent->gap.opcode)
    {
        ...
        case GAP_LINK_TERMINATED_EVENT:
        {
            ...
            if(!gPeriodicState && !discoveryFlag)
            {
                GAPRole_CentralStartDiscovery(DEFAULT_DISCOVERY_MODE,
                                              DEFAULT_DISCOVERY_ACTIVE_SCAN,
                                              DEFAULT_DISCOVERY_WHITE_LIST);
