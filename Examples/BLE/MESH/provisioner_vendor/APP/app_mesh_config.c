...
{
    tmos_memcpy(data, (uint8_t*)offset, len);
    return 0;
}
...
int write_flash(int offset, const void *data, unsigned int len)
{
    ...
    __attribute__((aligned(4))) uint8_t vec[64]={0};
    uint8_t i;
    ...
    for( i=0; i<(len+1)/2; i++)
    {
        status = FLASH_ProgramHalfWord(offset+i*2,vec[i*2]|(vec[i*2+1]<<8));
