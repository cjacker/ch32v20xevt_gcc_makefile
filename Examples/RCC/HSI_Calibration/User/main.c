...
volatile uint16_t RTC_DIV_VAL = 512;
...
int main(void)
{
...
#ifdef CH32V20x_D6
    if((*(uint32_t *)0x40022030) & 0x0F000000)
    RTC_DIV_VAL = 128;
#endif 
    ...
    do
    {
        ...
        {
            SampleFlag = 0;
            Meas_Freq = (uint32_t)((float)HSI_VALUE * ((float)(HSE_VALUE / RTC_DIV_VAL / 10) / (float)rtc_cnt));
            printf("HSI Freq: %d\r\n", Meas_Freq);
            ...
            Delay_Ms(100);
            last_Meas_Freq = (uint32_t)((float)HSI_VALUE * ((float)(HSE_VALUE / RTC_DIV_VAL / 10) / (float)rtc_cnt));
